...
namespace Brainx2\File\FileEntityBundle\Validator\Constraints;
 ...
 * @api
 */
class X2FileValidator extends ConstraintValidator
{
    const KB_BYTES = 1000;
        ...
        if(!is_string($value) && !is_null($value)) {
            ...
            if (!is_file($path)) {
                $this->buildViolation($constraint->notFoundMessage)
                    ...
                    ->setCode(X2File::NOT_FOUND_ERROR)
                    ->addViolation();
            ...
            if (!is_readable($path)) {
                $this->buildViolation($constraint->notReadableMessage)
                    ->setParameter('{{ file }}', $this->formatValue($path))
                    ->setCode(X2File::NOT_READABLE_ERROR)
                    ->addViolation();
            ...
            if ($constraint->maxSize) {
                ...
                if ($sizeInBytes > $limitInBytes) {
                    ...
                    $this->buildViolation($constraint->maxSizeMessage)
                        ...
                        ->setParameter('{{ limit }}', $limitAsString)
                        ->setParameter('{{ suffix }}', $suffix)
                        ->setCode(X2File::TOO_LARGE_ERROR)
                        ->addViolation();
            ...
            if ($constraint->mimeTypes) {
                ...
                $this->buildViolation($constraint->mimeTypesMessage)
                    ...
                    ->setCode(X2File::INVALID_MIME_TYPE_ERROR)
                    ->addViolation();
            }
        }
    }
